...
<div class="um <?php echo esc_attr( $this->get_class( $mode ) ); ?> um-<?php echo esc_attr( $form_id ); ?>">
	...
	<div class="um-form">
		...
		<form method="post" action="">
				...
				<ul>
					<?php
						if( $_SERVER['REQUEST_URI'] === '/account/' ) {
							UM()->account()->current_tab = 'welcome';
						}
					?>
          ...
          <li>
            <a class="um-tab-custom-link" href="<?php echo get_home_url() . '/reviews/'; ?>">
              <span class="um-account-icontip uimob800-show" title="<?php esc_attr_e('Review Program', 'flatsome'); ?>">
								<i class="um-faicon-star"></i>
							</span>
              <span class="um-account-icon uimob800-hide">
								<i class="um-faicon-star"></i>
							</span>
              <span class="um-account-title uimob800-hide"><?php esc_html_e('Review Program', 'flatsome'); ?></span>
            </a>
          </li>

          <li>
            <a class="um-tab-custom-link" href="<?php echo wp_logout_url(); ?>">
              <span class="um-account-icontip uimob800-show" title="<?php esc_attr_e('Logout', 'flatsome'); ?>">
								<i class="um-faicon-sign-out"></i>
							</span>
              <span class="um-account-icon uimob800-hide">
								<i class="um-faicon-sign-out"></i>
							</span>
              <span class="um-account-title uimob800-hide"><?php esc_html_e('Logout', 'flatsome'); ?></span>
            </a>
          </li>
				</ul>
			</div>
			...
			<div class="um-account-main" data-current_tab="<?php echo esc_attr( UM()->account()->current_tab ); ?>">
              ...
              if( $id == 'general' ) {
                ...
                echo '<div class="account-created">
                        ...
                        <div>' . date( "d M, Y", strtotime( $registered_date ) ) . '</div>
                      </div>';
              }
							// var_dump([$id]);
				// 			print_r($info);
        ...
        <!--div class="um-account-nav uimob340-show uimob500-show">
          <a class="um-tab-custom-link" href="<?php echo get_home_url() . '/rewards/'; ?>">
            <?php esc_html_e('Cashback Points', 'flatsome'); ?>
            <span class="ico"><i class="fas fa-dollar-sign"></i></span>
          </a>
        </div-->

        <!--div class="um-account-nav uimob340-show uimob500-show">
          <a class="um-tab-custom-link" href="#">
            <?php esc_html_e('Refer A Friend', 'flatsome'); ?>
            <span class="ico"><i class="fas fa-sync-alt"></i></span>
          </a>
        </div-->

        <div class="um-account-nav uimob340-show uimob500-show">
          <a class="um-tab-custom-link" href="<?php echo get_home_url() . '/reviews/'; ?>">
            <?php esc_html_e('Review Program', 'flatsome'); ?>
            <span class="ico"><i class="um-faicon-star"></i></span>
          </a>
        </div>

        <div class="um-account-nav uimob340-show uimob500-show">
          <a class="um-tab-custom-link" href="<?php echo wp_logout_url(); ?>">
            <?php esc_html_e('Logout', 'flatsome'); ?>
            <span class="ico"><i class="um-faicon-sign-out"></i></span>
          </a>
        </div>
