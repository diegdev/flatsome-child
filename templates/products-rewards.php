...
?>
<div class="products-rewards tabbed-content __total-point-<?php echo $rewards_able; ?>">
  ...
  <ul class="nav nav-line-bottom nav-uppercase nav-size-normal nav-center">
    <?php for($i = 1; $i <= $rewards_items; $i++){ ?>
      <li class="tab <?php echo $i == 1 ? 'active' : ''; ?> has-icon <?php echo $i <= $_user_rewards ? 'rewards_item_active' : ''; ?>">
        <a href="#tab_tab-<?php echo $i; ?>-title"><span><?php echo $i*10; ?></span><i class="um-faicon-star"></i></a>
      </li> 
    <?php } ?>
  </ul>
      ...
      <div class="panel <?php echo $i == 1 ? 'active' : ''; ?> entry-content" id="tab_tab-<?php echo $i; ?>-title">
        ...
        <div class="single_product row">
          ...
          <div class="single_product_right col medium-5 small-12 large-5">
              ...
              <div method="POST" action="/account/welcome/" class="claim_reward_form">
                ...
                <?php if(!bt_matched_cart_items($product_id)): ?>
                  <?php 
                    ...
                    if($in_stock) {
                      echo '<button class="btn button primary claim_reward" name="claim_reward" type="submit">Claim Reward</button>';
                    } else {
                      echo '<b style="background: #d2d2d2; color: #3d3d3d; border-radius: 0; text-transform: uppercase; padding: 8px 25px; margin-top: 1em; display: inline-block;">Out of stock</b>';
                    }
                  ?>
            ...
            <?php else: ?>
              <br />
              <a class="btn btn-gray btn-disable" href="javascript:"><?php _e('Claim Reward', '') ?></a>
            <?php endif; ?>
          </div>
        </div>
        <?php endif; ?>
...
<style>
  .products-rewards .woocommerce-Price-amount, 
  .products-rewards .product_price_discount{
    display: none;
  }
  
  /* .__total-point-0 .single_product{
    background-color: unset;
    display: flex;
    flex-wrap: wrap;
  }

  .__total-point-0 .single_product > div {
    width: 100%;
    max-width: unset; 
    flex-basis: auto;
  } */
