		...
		<td class="order-data">
			<table>
				...
				<?php
					...
					$order_count = 1;
					$_customer_order_count = get_post_meta( $order_id, '_customer_order_count', true );
					$_user_order_count = get_user_meta( $customer_id, '_user_order_count', true );
					if($_user_order_count){
						$_user_order_count = (array)json_decode($_user_order_count, true);
						$order_count = isset($_user_order_count[$order_id]) ? $_user_order_count[$order_id]['order_count'] : 1;
					}

				?>
				<tr class="total-order-method">
			    <th><?php _e( 'Total Order:', 'woocommerce-pdf-invoices-packing-slips' ); ?></th>
			    <td><?php echo $order_count;if(isset($_REQUEST['test'])): var_dump($_user_order_count); endif; ?></td>
			  </tr>
				<!-- <tr class="total-order-amount">
			    <th><?php _e( 'Order Amount:', 'woocommerce-pdf-invoices-packing-slips' ); ?></th>
